 ...
 * Toast Container
 *
 * Renders transient toast notifications from the UI store.
-->

<script lang="ts">
  import { uiStore } from '../../stores/ui';
  import { fly, fade } from 'svelte/transition';
  import { MOTION_FAST_MS } from '../../constants';
...
<div class="fixed bottom-4 right-4 z-[200] flex flex-col gap-2 w-80 pointer-events-none">
  {#each toasts as toast (toast.id)}
    <div 
      in:fly={{ y: 8, duration: MOTION_FAST_MS }} 
      out:fade={{ duration: MOTION_FAST_MS }}
      class="pointer-events-auto rounded-[var(--nt-dialog-radius)] shadow-elevated border border-subtle bg-elevated p-2 relative flex flex-col"
    >
      ...
      <div class="flex items-start gap-2 mt-0.5">
        {#if toast.type === 'success'}
          ...
          <div class="text-sm text-primary flex-1 break-words leading-snug pr-6">
            <span class="font-medium">{toast.title}</span>
            {#if toast.message}
              <div class="mt-0.5 opacity-80 text-xs">{toast.message}</div>
            {/if}
          </div>
        ...
        {:else}
          ...
          <div class="flex-1 min-w-0 pr-6">
            {#if toast.message}
                <button 
                type="button"
                class="flex items-center gap-2 group select-none w-full text-left" 
              ...
              >
                ...
                <div class="text-icon-default group-hover:text-icon-active {expandedToasts.has(toast.id) ? 'rotate-90' : ''}" aria-hidden="true">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>
                </div>
              </button>
              {#if expandedToasts.has(toast.id)}
                <div class="text-xs text-primary opacity-80 mt-1 bg-surface-3 p-1.5 rounded-[2px] break-words whitespace-pre-wrap max-h-40 overflow-y-auto">
                  {toast.message}
                </div>
              {/if}
            {:else}
