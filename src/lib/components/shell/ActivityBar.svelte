...
<div class="w-12 flex flex-col items-center py-0 justify-between z-10 border-r bg-[var(--nt-activitybar-bg)] border-[var(--nt-activitybar-border)]">
  <div class="flex flex-col items-center gap-1 w-full mt-2">
    {#each items as item (item.id)}
      ...
      <Tooltip content={item.tooltip} side="right" hoverDelay={300}>
        <button
          aria-label={item.label}
          onclick={() => handleClick(item)}
          class="p-2 mb-2 bg-transparent cursor-pointer relative transition-colors hover:text-icon-active"
  ...
  <Tooltip content="Settings (Ctrl+,)" side="right" hoverDelay={300}>
    <button
      ...
      class="p-2 bg-transparent cursor-pointer mb-2 text-icon-default hover:text-icon-active transition-colors"
    >
